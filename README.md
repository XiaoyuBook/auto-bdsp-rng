...
当前正式版为 [v3.1.0](https://github.com/XiaoyuBook/auto-bdsp-rng/releases/tag/v3.1.0)。普通用户请不要下载 GitHub 绿色 `Code` 按钮中的源码压缩包，请下载 Windows x64 绿色版：

```text
auto-bdsp-rng-v3.1.0-windows-x64.zip
```
...
`v3.0.0` 是首个内置升级器的正式版。已安装带升级器正式版的用户可通过“帮助 -> 检查更新…”升级，软件会优先使用可用的增量升级链；若提示没有升级链，请下载完整压缩包。`v2.1.7` 及更早版本需要完整下载一次上述 zip。

下文功能说明以已发布的 `v3.1.0` 为基础。当前 `main` 源码已更新至 `v3.1.1`，新版 Windows 包待发布；本轮改善视频源连接、残留进程恢复与采集格式协商，详见 [v3.1.1 更新日志](CHANGELOG.md#311)。
...
| 2026-09-04 | 发布 `v3.0.3`：集中配置自动定点校正阈值、重试次数和失败补救，并加入 8 种逐轮冻结、可持久学习反查样本的 delay 策略。 |
| 2026-09-07 | 发布 `v3.0.4`：按精灵隔离并持久化 delay 策略与反查样本，完成确认版紧凑工作区和交互/视觉校准，并修复长时间运行视频预览偶发误断联。 |
| 2026-09-09 至 09-11 | 发布 `v3.1.0`：完善六页原生界面、MiSans 字体、窄屏布局和表格操作，拆分参数/脚本保存；校正未命中后扩大搜索范围，并完成 25 万/50 万/90 万帧 Project_Xs 对照；TID 表增加累计用时、固定日期、取名倒计时及带数量的目标筛选与定位。 |
| 2026-09-14 | 源码更新至 `v3.1.1`（Windows 包待发布）：修复残留视频源记录导致的连接失败，改善 Media Foundation 与 DirectShow 采集兼容性，分开处理启动、设备打开与首帧超时，并补充实际采集参数日志。 |
