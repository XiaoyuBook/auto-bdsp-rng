...
- 支持键盘虚拟手柄、按键映射、按下/释放和摇杆输入；映射窗口中可右键对应按键并选择“删除映射”。Windows 下启用后可在软件失去焦点时继续控制。按 `Esc` 会立即在启用与待机之间切换，按 `Ctrl+Esc` 会彻底关闭并隐藏状态窗口；待机时普通映射键正常透传，仅保留快捷键监听。状态窗口复用原版 EasyCon 的 `100x100` VPad 素材与绘制坐标，会显示实际发送给单片机的按键和摇杆报告，也能显示脚本操作。
- 连接伊机控并启用键盘虚拟手柄后，可录制操作和等待时间，支持暂停/继续并把结果插入编辑器。开始页面脚本或自动流程时会停用键盘控制并释放当前输入，脚本结束后需由用户手动重新启用。
