...
| 2026-05-12 | 集成 C++ 原生 RNG 扩展（pybind11），搜索性能提升 ~200 倍；修复早期拒绝导致 RNG 序列截断；修复反查 IV 公式缺少 +5 的 bug，改用 PokeFinder Nature::computeStat；IV 计算器统一使用 C++ PokeFinder 公式；修复 delay 三处定义错误（runner / 反查日志 / 历史面板）；所有只读文本视图支持右键「复制」+ Ctrl+C；伊机控默认启用 CLI 模式；历史记录候选改为单行紧凑格式并标注同步来源；单次模式无候选时直接结束不再死循环。 |
