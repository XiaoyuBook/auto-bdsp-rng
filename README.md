...
当前正式版为 [v3.0.4](https://github.com/XiaoyuBook/auto-bdsp-rng/releases/tag/v3.0.4)。普通用户请不要下载 GitHub 绿色 `Code` 按钮中的源码压缩包，请下载 Windows x64 绿色版：

```text
auto-bdsp-rng-v3.0.4-windows-x64.zip
```
...
`v3.0.0` 是首个内置升级器的正式版。已安装 `v3.0.0`、`v3.0.1`、`v3.0.2` 或 `v3.0.3` 的用户可以直接在软件中检查并升级到 `v3.0.4`，无需重新下载完整压缩包；`v2.1.7` 及更早版本仍需要完整下载一次上述 zip。

下文功能说明对应当前 `main` 源码 `v3.1.0`，新版 Windows 包待发布。TID 时间表、校正搜索回退和最新界面调整尚未包含在已发布的 `v3.0.4` 包中，详见 [v3.1.0 更新日志](CHANGELOG.md#310)，其中也汇总了 `v3.0.4` 的更新。
...
| 2026-09-09 至 09-11 | 源码更新至 `v3.1.0`（Windows 包待发布）：完善六页原生界面、MiSans 字体、窄屏布局和表格操作，拆分参数/脚本保存；校正未命中后扩大搜索范围，并完成 25 万/50 万/90 万帧 Project_Xs 对照；TID 表增加累计用时、固定日期、取名倒计时及带数量的目标筛选与定位。 |
