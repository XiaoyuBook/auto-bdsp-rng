...
- 动态样本只来自成功且有候选的自动反查，每轮候选去重后成组保存，并记录稳定轮次号和时间。设置窗口显示最近 5 轮，完整记录每页 10 轮；单轮可划除并用回转箭头恢复，划除记录保留在原位但不参与计算，也可清空当前精灵的全部样本。
- 多候选轮次默认忽略，也可设置为每轮总权重为 1、由轮内候选均分；“上次实际 delay”始终跳过多候选轮，其他统计策略使用最近 N 个有效轮次。
