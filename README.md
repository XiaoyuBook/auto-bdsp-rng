...
- “自动策略”中的 delay 摘要按钮可选择固定 delay、上次实际 delay、众数、中位数、滚动平均值、指数平滑、截尾平均和密集区间 8 种策略。默认继续使用固定策略；原 `fixed_delay` 设置作为基准 delay 保留，并在动态策略没有有效样本时回退使用。
- 动态样本只来自成功且有候选的自动反查，每轮候选去重后成组保存。多候选轮次默认忽略，也可设置为每轮总权重为 1、由轮内候选均分；“上次实际 delay”始终跳过多候选轮，其他统计策略使用最近 N 个有效轮次。
- 每个新轮次开始时会计算并冻结本轮 delay；同轮的重搜、逃跑续搜和补救测 Seed 都继续使用该值。运行中修改策略或反查得到新样本只影响下一轮，样本会跨启动保存，也可在设置窗口中清空。
