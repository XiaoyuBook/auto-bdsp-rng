...
配置和样本通过 `DelayProfile` 按物种编号保存，不按脚本版本或性格、个体值等筛选条件分组。每个 profile 包含完整策略配置、带稳定正整数轮次号和本地 ISO 时间的样本，以及下一个轮次号；JSON 使用带版本号的 schema，单个损坏物种只丢弃该 profile。升级旧设置时必须先恢复目标，再把旧全局配置和样本迁移到当时的目标物种；目标记录损坏时仍激活表单中的默认物种，但不迁移无法确认归属的旧样本。

设置窗口显示当前精灵、本轮冻结值和下轮预计值。最近列表固定展示新到旧的 5 轮，完整记录按轮次倒序每页 10 轮，并可选显示记录时间。单轮“删除”是持久化软删除：前三列保留并加删除线，操作按钮改为回转箭头；恢复后重新参与计算。划除、恢复和清空只重算下轮预计值。本轮运行服务始终携带启动配置中的物种编号，即使运行中切换 UI 目标，解析 delay 和写入反查样本仍落在启动物种的 profile。

推荐 delay 是按物种维护的可选参考值。没有可信数据的物种显示“暂无推荐”；点击“使用推荐值”只修改弹窗中的基准值草稿，保存后才写入用户 profile，后续推荐值更新也不覆盖用户已保存设置。
...
- `校正策略设置...` 按钮：打开模态设置窗口，不在主表单逐项展开以下参数。
  ...
  - 重测 Seed 最大尝试次数：默认 1，最小 1，始终可预先编辑，仅在选择补救策略时生效。
