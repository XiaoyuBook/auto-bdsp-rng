...
| `fixed_delay` | 用户填写的固定 delay，用来提前运行撞闪脚本 | 100 |
| `trigger_advances` | 撞闪脚本理论启动帧，`raw_target_advances - fixed_delay` | 900 |
...
- 进入撞闪阶段后，必须做最终实时校准；给撞闪脚本填 `_闪帧 = flash_frames`，其中 `flash_frames` 是脚本即将启动时重新计算出的实时剩余帧。
...
`fixed_delay` 不参与 seed 搜索，也不修改当前 advances。它只用于把撞闪脚本的启动点从目标帧往前移动。

严格公式：

```text
raw_target_advances = state.advances
trigger_advances = raw_target_advances - fixed_delay
...
current_advances = 0 时，remaining_to_trigger = 900，需要先过帧。
current_advances = 600 时，remaining_to_trigger = 300，可以进入最终撞闪等待区。
```
...
`_闪帧` 的语义不是“上一步搜索时还剩多少帧”，而是“撞闪脚本真正开始执行的那一刻，距离目标触发点还剩多少 advances”。

因此进入 `max_wait_frames` 范围后，不能直接把旧的 `remaining_to_trigger` 填进去运行。必须执行最终实时校准：

1. 做一次 final reidentify，或在需要重新测 seed 的场景做 final capture seed。
2. 用最新 seed / current advances 重新搜索或重新确认锁定目标。
3. 得到 `trigger_advances = raw_target_advances - fixed_delay`。
4. 记录校准参考点：`current_advances_at_ref` 和 `ref_time`。
5. 在即将生成并提交撞闪脚本前，用当前时间修正已经流逝的 advances。
6. 计算 `flash_frames = trigger_advances - live_current_advances`。
7. 填 `_闪帧 = flash_frames` 后立即运行撞闪脚本。
...
第一版安全规则：
- `flash_frames <= 0`：已错过目标，不运行撞闪脚本。
- `flash_frames < min_final_flash_frames`：距离太近，脚本启动和通信误差可能导致错过；放弃本目标并回到测 seed / 搜索流程。
...
- 每一步写明输入和决策结果，例如：
  ...
  - `final reidentify current=602，实时修正后 flash_frames=298，运行撞闪脚本`
...
stateDiagram-v2
    ...
    FinalCalibrate --> RunHitScript: flash_frames safe
...
- 开始前校验过帧脚本包含 `_目标帧数`。
- 开始前校验撞闪脚本包含 `_闪帧`。
- `raw_target_advances <= fixed_delay` 时不允许启动撞闪，提示 delay 过大。
- `remaining_to_trigger <= 0` 时判定已错过目标，不运行撞闪。
- 最终校准后的 `flash_frames <= 0` 时判定已错过目标，不运行撞闪。
- 最终校准后的 `flash_frames < min_final_flash_frames` 时判定距离太近，放弃本目标。
