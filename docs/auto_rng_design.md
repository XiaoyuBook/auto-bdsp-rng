...
| `fixed_flash_frames` | 撞闪脚本内固定写死的 `_闪帧`；自动流程不再动态改写 | 60 |
...
`_闪帧` 不再由自动流程按实时剩余帧动态填写。它是撞闪脚本内固定的小等待窗口，默认 60；用户通过多次实机运行校准 `fixed_delay`，软件根据实时 advances 决定脚本启动帧。
...
3. 得到 `trigger_advances = raw_target_advances - fixed_delay - fixed_flash_frames`。
