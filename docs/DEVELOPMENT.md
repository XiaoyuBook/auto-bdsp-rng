...
运行 `run_dev_gui.bat`，或在源码入口使用以下命令，可显示 Mock 视频源与 Mock 串口：

```powershell
.\.venv\Scripts\python.exe -m auto_bdsp_rng gui --mock-devices
```

在相应连接窗口中选择并连接 Mock 设备后，可测试键盘控制、录制、脚本执行跟随及引导交互。手动捕捉 Seed 或校正时可选择模拟成功/失败，检查对应分支。Mock 视频为静态演示画面，不能据此验证真实眨眼、实机时序或乱数结果；正式包的普通启动入口不启用这些设备。
